...
HARVESTER = (
    ...
    "if(window.__dseHarvest){alert('Already running - watch the badge.');return;}"
    "window.__dseHarvest=1;"
    "const cards=[...document.querySelectorAll('a.team-card')];"
    "if(!cards.length){window.__dseHarvest=0;"
    "alert('Run this on the team directory page.');return;}"
    "const box=document.createElement('div');"
    "box.style.cssText='position:fixed;top:12px;right:12px;z-index:999999;"
    "background:#111;color:#fff;font:600 14px system-ui,sans-serif;"
    "padding:10px 14px;border-radius:8px;box-shadow:0 2px 12px rgba(0,0,0,.4)';"
    "box.textContent='Harvesting 0/'+cards.length;"
    "document.body.appendChild(box);"
    "const out=[];let done=0,empty=0;"
    ...
    "if(!arts.length)empty++;"
    "out.push('<!-- DSE-TEAM | '+id+' | '+nm.trim()+' | '+dv+' -->');"
    "out.push(arts.join('\\n'));}catch(e){empty++;}"
    "done++;box.textContent='Harvesting '+done+'/'+cards.length+' - '+nm.trim().slice(0,24);"
    "await new Promise(r=>setTimeout(r,150));}"
    "const blob=new Blob([out.join('\\n')],{type:'text/html'});"
    "const a=document.createElement('a');a.href=URL.createObjectURL(blob);"
    "a.download='dse_rosters.html';a.click();"
    "box.style.background='#137333';"
    "box.textContent='Done: '+done+' teams'+(empty?' ('+empty+' empty)':'')"
    "+' - check Downloads';"
    "setTimeout(function(){box.remove();window.__dseHarvest=0;},15000);})()"
)
